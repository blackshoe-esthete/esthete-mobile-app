...
import {
  ExhibitionDetailsStore,
  ImageItem,
} from "../types/exhibitionService.type";
...
export const useExhibitionCreationStore = create<ExhibitionCreationStore>(
  (set, get) => ({
...
export const useExhibitionDetailsStore = create<ExhibitionDetailsStore>(
  (set) => ({
    details: {
      filterPhotos: [],
      title: "",
...
interface FilterDetailsStore {
  ...
  updateFilterAttributes: (
    id: string,
    attributes: Partial<FilterAttributes>
  ) => void;
...
export const useFilterDetailsStore = create<FilterDetailsStore>((set, get) => ({
  ...
  setSelectedFilterAttributes: (attributes) =>
    set({ selectedFilterAttributes: attributes }),
  setCurrentFilterAttributes: (attributes) =>
    set({ currentFilterAttributes: attributes }),
  updateFilterAttributes: (id, attributes) =>
    set((state) => {
      const updatedFilters = state.filters.map((filter) =>
        filter.filter_id === id
          ? { ...filter, attributes: { ...filter.attributes, ...attributes } }
          : filter
      );
