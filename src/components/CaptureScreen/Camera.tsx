...
import Animated, {useSharedValue, interpolate, Extrapolation, useAnimatedProps, runOnJS} from 'react-native-reanimated';
...
const CameraFn = forwardRef<Camera, Props>((props: any, ref: any): React.JSX.Element => {
  ...
  const pinchGesture = Gesture.Pinch()
    ...
    .onUpdate(event => {
      ...
      zoom.value = interpolate(z, [1, 10], [device?.minZoom ?? 1, device?.maxZoom ?? 16], Extrapolation.CLAMP);
    });
        ...
        <GestureDetector gesture={composed}>
          <Animated.View onTouchEnd={onFocusTap} style={{flex: 0.8}}>
            <ReanimatedCamera
              ...
              //isActive={true}
              isActive={props.onInitialized && !photo && isFocused && isForeground}
              photo={true}
